  ...
  if (args.has("--fixture")) {
    ...
    if (args.has("--write-fixture")) {
      const fx = {
        ...
        source: { region: process.env.EXACT_REGION ?? "be", division: await division(token), observed: new Date().toISOString().slice(0, 10), method: "$select probes, plus a sample row where one exists", probed: live.probed },
